...
class RespiratoryReconstructSSAM:
  ...
  def objFuncAirway(self, pose, scale=None, b=None):
    ...
    density_t = self.getDensity(all_morphed, self.img, self.imgCoords)
    ...
    top_dist = 1.0 - np.exp(
      -1.0 * abs(tallest_pt[2] - self.imgCoords[:, 1].max()) / 5.0
    )
