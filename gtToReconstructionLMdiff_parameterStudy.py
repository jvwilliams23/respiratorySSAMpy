...
for p in params:
 paramVals.append(p)
file_args = paramVals+[caseID, 'ALL']
file_name_args = file_name.format(*file_args)


lm_index_file = 'allLandmarks/landmarkIndexAirway.txt'
airway_lm_index = np.loadtxt(lm_index_file).astype(int)
out_name = 'outputLandmarks/' + file_name_args
...
np.savetxt("morphologicalAnalysis/landmarkDistanceStats{}.txt".format(file_name_args),
          dist,
          header="distance [mm]",
          fmt="%4f")
...
np.savetxt("morphologicalAnalysis/lengthStats{}.txt".format(file_name_args),
...
np.savetxt("morphologicalAnalysis/diameterStats{}.txt".format(file_name_args),
...
np.savetxt("morphologicalAnalysis/angleStats{}.txt".format(file_name_args),
