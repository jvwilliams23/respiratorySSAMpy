...
class RespiratorySAM:
  ...
  def __init__(self, lm, imgs, imgsOrigin, imgsSpacing, train_size=0.9):
    ...
    self.imgCoords = self.drrArrToRealWorld(self.imgs, imgsOrigin, imgsSpacing)

    # test landmarks are within image
    # self.testLandmarkAndImageBounds(self.lm[:,:,[0,2]], self.imgCoords)

    self.density_base = self.getTrainingDensity(self.lm, self.imgs, self.imgCoords)
    ...
    self.density = (self.density_base
                    -self.density_base.mean(axis=1)[:, np.newaxis]
                    )
    ...
    if __name__=="__main__":
      self.pca, k_95 = self.doPCA(self.density, 0.95) #-train the model and get PCs
  ...
  def getDensity(self, lm, img, imgCoords):
    ...
    nearestX = np.argmin(abs(lm[:,0]
                             -imgCoords[:,0].reshape(-1,1)), 
                          axis=0)
    nearestZ = np.argmin(abs(lm[:,2]
                             -imgCoords[:,1].reshape(-1,1)), 
                          axis=0)
  ...
  def getTrainingDensity(self, lm, img, imgCoords):
    ...
    for p in range(lm.shape[0]):
      if debug: print("testing {} patient {}".format(shape, patientIDs[p]))
  ...
  def normaliseImageDensity(self, density_im):
    ...
    densityN_im = (density_im
                    -density_im.mean(axis=(1,2))[:,np.newaxis, np.newaxis]
                    )
    densityN_im = densityN_im/density_im.std(axis=(1,2))[:,np.newaxis, np.newaxis]
  ...
  def denormaliseLandmarkDensity(self, densityN_lm, density_im):
    ...
    else: printc("error in denormaliseLandmarkDensity function, SAM.py",
...
def getInputs():
  parser = argparse.ArgumentParser(description='statistical appearance model class')
  ...
  if getModes:
    ...
    for i, mode in enumerate(modes):
      ...
      for j, std in enumerate(stds):
        ...
        a = ax[j][i].scatter(meanShape[:,0], meanShape[:,2], 
    ...
    fig.text(0.5, 0.95, "appearance components",
            horizontalalignment='center', fontsize=12)
    fig.savefig("images/SAM/SAM-densityModes-diff-meanShape.png", 
                 pad_inches=0, format="png", dpi=300)
    ...
    fig.text(0.5, 0.95, "appearance components",
            horizontalalignment='center', fontsize=12)
    fig.savefig("images/SAM/SAM-densityModes-meanShape.png", 
                 pad_inches=0, format="png", dpi=300)
