...
class RespiratorySSAM:
  def __init__(
    ...
    **kwargs,
    ...
    self.x_vec = self.lm.reshape(
      self.lm.shape[0], self.lm.shape[1] * self.lm.shape[2]
    )
    ...
    self.xg = np.dstack((self.x_vec_scale.reshape(self.lm.shape), self.density))
    if imgs.ndim == 3:
      self.xg_vec = self.xg.reshape(
        self.xg.shape[0], self.xg.shape[1] * self.xg.shape[2]
      )
    elif imgs.ndim == 4:
      self.xg_vec = self.xg.reshape(
        self.xg.shape[0], self.xg.shape[1] * self.xg.shape[2]
    ...
    self.xg_train = self.xg_train.reshape(
      -1, self.xg.shape[1], self.xg.shape[2]
    )
...
def getInputs():
  ...
  parser.add_argument(
    "--getModes", default=False, type=str, help="write main modes of variation"
  )
  parser.add_argument(
    "--debug",
    default=False,
  ...
  assert path.isdir(landmarkDir) and path.isdir(
    drrDir
  ), "data directories do not exist"
...
if __name__ == "__main__":
  ...
  print("shape and appearance modelling")
  ...
  assert not (
    ...
    f"\n {drrDir} {len(imDirs)}"
    f"\n {landmarkDir} {len(landmarkDirs)}"
    f"\n landmarks/manual-jw/ {len(landmarkDirsOrig)}"
  )
  ...
  if getModes:
    print("plotting modes of shape + appearance variation...")
    modes = [0, 1, 2, 3]
    stds = [3, -3]
    ...
    fig.text(
      0.5,
      0.95,
      "shape & appearance components",
      horizontalalignment="center",
      fontsize=12,
    )
    fig.savefig(
      "images/SSAM/SSAM-modes-diff.png", pad_inches=0, format="png", dpi=300
    )
    ...
    for i, mode in enumerate(modes):
      ...
      for j, std in enumerate(stds):
        ...
        modeIm = (
          var[:, -1] * ssam.sam.density_base.std(axis=0)
    ...
    fig.savefig(
      "images/SSAM//SSAM-modes.png", pad_inches=0, format="png", dpi=300
    )
