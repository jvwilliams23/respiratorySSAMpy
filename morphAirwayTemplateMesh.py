...
python morphAirwayTemplateMesh.py --plot y \
      --templateMesh segmentations/template3948/newtemplate3948_mm.stl \
      --templateLMs landmarks/manual-jw-diameterFromSurface/landmarks3948_diameterFromSurf.csv \
      --morphLMs landmarks/manual-jw-diameterFromSurface/landmarks5837_diameterFromSurf.csv  \
      --case 5837 -gt segmentations/airwaysForRadiologistWSurf/5837/5837-segmentation.stl \
      --sigma 0.1
"""
...
if __name__ == "__main__":
  ...
  for lmFile in landmarkFilesOrig:
    if case in lmFile:
      carinaNew = np.loadtxt(lmFile, skiprows=1, delimiter=',',
                                      usecols=[1,2,3])[1]*-1
    if '3948' in lmFile:
      carinaTemplate = np.loadtxt(lmFile, skiprows=1, delimiter=',',
                                      usecols=[1,2,3])[1]*-1
      print(carinaTemplate)

  # load and pre-process data
  mesh_template = v.load(template_mesh)
  mesh_template = mesh_template.clone().pos(carinaTemplate)
  # if coarsen_nodes != 1:
  #   mesh_template.decimate(fraction=1/coarsen_nodes) 
  # coords_template = mesh_template.points()
  # coords_template -= coords_template.mean(axis=0) # align mesh to landmarks
  lm_template = np.loadtxt(template_lm, delimiter=",", skiprows=1)[::coarsen_lms]
  lm_target = np.loadtxt(target_lm, delimiter=",", skiprows=1)[::coarsen_lms]

  morph = MorphAirwayTemplateMesh(lm_template, lm_target, mesh_template,
  ...
  if ground_truth:
    mesh_true = v.load(ground_truth).pos(carinaNew).alpha(0.3)
